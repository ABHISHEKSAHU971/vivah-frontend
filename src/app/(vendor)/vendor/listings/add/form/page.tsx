          ...
          <div className="space-y-4">
            {detailForm.packages?.map((pkg: any, idx: number) => (
              <div key={idx} className="p-4 border border-gray-200 rounded-2xl bg-zinc-50/30 space-y-4 relative animate-fade-in">
                ...
                <div className="space-y-1.5">
                  <label className="text-[10px] font-bold uppercase tracking-widest text-gray-400 block">Included Menu Items Summary</label>
                  <textarea
                    value={pkg.description}
                    rows={2}
                    placeholder="Briefly describe what makes this plan unique (e.g. 3 Starters, 4 Mains, 2 Desserts)..."
                    onChange={(e) => {
                      const list = [...detailForm.packages];
                      list[idx].description = e.target.value;
                      handleDetailChange("packages", list);
                    }}
                    className="w-full border border-gray-200 rounded-xl px-4 py-2 text-xs bg-white text-gray-900 focus:outline-none resize-none"
                  />
                </div>
              </div>
            ))}
            ...
            <button
              ...
              onClick={() => {
                const list = [
                  ...(detailForm.packages || []),
                  { name: "", cuisine_type: "multi", tier: "medium", price_per_plate: "", min_plates: 80, description: "" }
                ];
                handleDetailChange("packages", list);
              }}
