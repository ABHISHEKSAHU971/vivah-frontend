...
function EditListingForm() {
  ...
  useEffect(() => {
    ...
    api.get(`/listings/${id}/`)
      .then((res) => {
        ...
        // DJ details come back as an array of packages — normalize to { packages: [...] }
  ...
  if (fetching) {
    return (
      <div className="min-h-[40vh] flex flex-col items-center justify-center gap-3 text-gray-400 font-body">
        <Loader2 size={32} className="animate-spin text-gold" />
        <p className="text-sm">Fetching listing detail parameters...</p>
      </div>
    );
  }
        ...
        <div className="bg-white border border-gray-100 rounded-2xl p-6 shadow-sm space-y-5">
          ...
          {type === "makeup" && (
            <div className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-1.5">
                  <label className="text-[10px] font-bold uppercase tracking-widest text-gray-400 block">Guest/Party Makeup Price (₹)</label>
                  <input
                    type="number"
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <label htmlFor="is_on_location" className="text-xs text-gray-600 font-semibold cursor-pointer">
                    Available to travel to client venue
                  </label>
                </div>
              </div>
          ...
          {type === "planner" && (
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <label className="text-[10px] font-bold uppercase tracking-widest text-gray-400 block">Min Managed Wedding Budget (₹)</label>
                  <input
                    type="number"
                ...
                <div className="space-y-1.5">
                  <label className="text-[10px] font-bold uppercase tracking-widest text-gray-400 block">Max Managed Wedding Budget (₹)</label>
                  <input
                    type="number"
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-6">
                {(detailForm.packages || []).map((pkg: any, idx: number) => (
                  <div key={idx} className="p-5 border border-gray-250 rounded-2xl bg-zinc-50/50 space-y-4 relative">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="space-y-1.5">
                        ...
                        <input
                          type="text"
                          value={pkg.name || ""}
                          placeholder="e.g. Standard Sound System & Lights"
                          onChange={(e) => {
                    ...
                    {/* Nested Equipment Loadout Manager */}
                    <div className="space-y-4 pt-4 border-t border-gray-200/60">
                      ...
                      <div className="p-3.5 border border-gold/15 rounded-xl grid grid-cols-1 sm:grid-cols-12 gap-3 items-end bg-[#C9A440]/5">
                        <div className="sm:col-span-3 space-y-1">
                          <label className="text-[9px] font-bold text-gray-400 uppercase block">Equipment / Item Name</label>
                          <select
                            id={`edit-dj-eq-preset-${idx}`}
                            defaultValue="Sharpy Light"
                            onChange={(e) => {
                              const customDiv = document.getElementById(`edit-dj-eq-custom-div-${idx}`);
                              const customInput = document.getElementById(`edit-dj-eq-custom-name-${idx}`) as HTMLInputElement;
                              if (customDiv) {
                        ...
                        <div id={`edit-dj-eq-custom-div-${idx}`} className="sm:col-span-3 space-y-1 hidden">
                          <label className="text-[9px] font-bold text-gray-400 uppercase block">Custom Equipment Name</label>
                          <input type="text" id={`edit-dj-eq-custom-name-${idx}`} placeholder="e.g. JBL Dual Top"
                        ...
                        <div className="sm:col-span-2 space-y-1">
                          ...
                          <input type="number" id={`edit-dj-eq-qty-${idx}`} min={0} defaultValue={1}
                            className="w-full border border-gray-200 rounded-lg px-2.5 py-1.5 text-xs bg-white text-gray-900 focus:outline-none" />
                        </div>

                        <div className="sm:col-span-2 space-y-1">
                          <label className="text-[9px] font-bold text-gray-400 uppercase block">Total Stock</label>
                          <input type="number" id={`edit-dj-eq-stock-${idx}`} min={1} defaultValue={5}
                        ...
                        <div className="sm:col-span-2 space-y-1">
                          ...
                          <select id={`edit-dj-eq-included-${idx}`} defaultValue="true"
                            onChange={(e) => {
                              const priceInput = document.getElementById(`edit-dj-eq-price-${idx}`) as HTMLInputElement;
                              if (priceInput) {
                                priceInput.disabled = e.target.value === "true";
                                if (e.target.value === "true") priceInput.value = "";
                              }
                            }}
                            className="w-full border border-gray-200 rounded-lg px-2.5 py-1.5 text-xs bg-white text-gray-900 focus:outline-none">
                        ...
                        <div className="sm:col-span-2 space-y-1">
                          ...
                          <input type="number" id={`edit-dj-eq-price-${idx}`} disabled placeholder="Free"
                        ...
                        <div className="sm:col-span-12 md:col-span-1 pt-2 sm:pt-0">
                          <button
                            type="button"
                            onClick={() => {
                              const presetEl = document.getElementById(`edit-dj-eq-preset-${idx}`) as HTMLSelectElement;
                              const customEl = document.getElementById(`edit-dj-eq-custom-name-${idx}`) as HTMLInputElement;
                              const qtyEl = document.getElementById(`edit-dj-eq-qty-${idx}`) as HTMLInputElement;
                              const stockEl = document.getElementById(`edit-dj-eq-stock-${idx}`) as HTMLInputElement;
                              const includedEl = document.getElementById(`edit-dj-eq-included-${idx}`) as HTMLSelectElement;
                              const priceEl = document.getElementById(`edit-dj-eq-price-${idx}`) as HTMLInputElement;
                              ...
                              const list = [...detailForm.packages];
                              list[idx].equipment = [...current, { item_name: eqName, quantity, quantity_available, is_included, unit_price }];
                              handleDetailChange("packages", list);

                              if (customEl) customEl.value = "";
                              qtyEl.value = "1"; stockEl.value = "5";
                              includedEl.value = "true"; priceEl.value = ""; priceEl.disabled = true;
                        ...
                        <table className="w-full text-left text-xs border-collapse">
                          ...
                          <tbody>
                            {(!pkg.equipment || pkg.equipment.length === 0) ? (
                              <tr><td colSpan={6} className="p-4 text-center text-gray-400 italic">No equipment loadout configured yet. Add items above.</td></tr>
                            ) : (
                                ...
                                <tr key={eqIdx} className="border-b border-gray-100 hover:bg-slate-50/50 transition-colors">
                                  ...
                                  <td className="p-3 text-center">
                                    <span className={`inline-block px-2 py-0.5 rounded-full text-[10px] font-bold ${
                                      eq.is_included ? "bg-green-50 text-green-600 border border-green-100" : "bg-amber-50 text-amber-600 border border-amber-100"
                                    }`}>{eq.is_included ? "Included" : "Extra Charge"}</span>
                                  </td>
  ...
  return (
    <Suspense fallback={
      <div className="min-h-[40vh] flex flex-col items-center justify-center gap-3 text-gray-400">
        <Loader2 size={32} className="animate-spin text-gold" />
        <p className="text-sm">Loading edit listing form...</p>
      </div>
    }>
      <EditListingForm />
    </Suspense>
